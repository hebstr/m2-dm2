...
format:
  hebstr-html:
    code-tools:
      source: https://github.com/hebstr/m2-dm2
---
...
.subtitle_hdrs <- str_glue(
  "Parmi les {nrow(hdrs$item$j0)} sujets évalués à J0,
...
.title_hdrs_items <-
  map(
    c(n2 = 2, n4 = 4),
    ~ str_glue(
      "Distribution des items cotés de 0 à {.} sur l'échelle HDRS aux
           temps {str_color('J0', .pal[1])} et {str_color('J56', .pal[2])}."
    )
  )
```

{{< script scripts/fig_hdrs.R >}}
...
include_graphics("output/fig_hdrs_items_n2.png")
```
...
#| lightbox:
#|   group: fig-hdrs-items
include_graphics("output/fig_hdrs_items_n4.png")
```
...
.title_hdrs_global <-
  str_fig(
    title = "Distribution du score global sur l'échelle HDRS aux temps
          {str_color('J0', .pal[1])} et {str_color('J56', .pal[2])}.",
    note = "Le score global provenait de l'addition des scores obtenus
...
{{< script scripts/fig_scree.R >}}

```{r}
#| label: fig-scree
#| out-width: 75%
#| fig-cap: !expr '.title_scree'
.title_scree <- str_glue(
  "Pourcentage de variance capturée par chaque composante, échelle HDRS à
     {str_color('J0', .pal[1])} et {str_color('J56', .pal[2])}."
)
...
.title_contrib <- list(
  pc = str_glue(
    "{str_color('Pourcentage de contribution', opts$color$w[2])} des items
     de l'échelle HDRS à chaque composante principale (PC), à J0."
  ),
  sort = list(
    J0 = "aux composantes principales (PC)",
    J56 = "à la première composante principale"
  ) |>
    imap(
      ~ str_glue(
        "Classement des items de l'échelle HDRS selon leur
...
{{< script scripts/fig_pca.R >}}

```{r}
#| label: fig-pca
#| fig-cap: !expr '.title_pca'
.title_pca <- str_fig(
  title = "Analyse en composantes principales sur la base du score HDRS,
           par item, de {str_color('J0', colorspace::darken(.ind$color$low, 0.07))}
           à {str_color('J56', .ind$color$high)}.",
  note = "Les sujets étaient évalués sur 17 items à {.ind$j_str}. {.subtitle_hdrs}
...
```{r}
.title_value <- list(
  ab = c("aberrantes", "fréquence"),
  na = c("manquantes", "visite")
) |>
  imap(
    ~ list(
      title = str_glue(
        "Distribution des valeurs {.x[1]} dans l'échelle SCL90, par {.x[2]}."
      ),
      note = str_glue(
        "Le recueil comportait {scl$n$col} items pour un total de
        {nrow(scl$base)} visites parmi {scl$n$row} sujets, soit un
        total de {scl$n$rc} items. Au total, {nrow(.value_data[[.y]]$n)}
        ({.value_data[[.y]]$p}) valeurs {.x[1]} étaient retrouvées.
        {.value_data[[.y]]$obs$n} sujets sur {scl$n$row} ({.value_data[[.y]]$obs$p})
        et {.value_data[[.y]]$item$n} items distincts sur {scl$n$col}
        ({.value_data[[.y]]$item$p}) étaient concernés par au moins une
...
Il y avait un total de **`r nrow(.value_data$ab$n)` (`r .value_data$ab$p`) valeurs aberrantes**, dont voici le détail :

{{< script scripts/fig_value.R >}}
...
#| lightbox:
#|   group: fig-value
.title_value_ab <- str_fig(
  title = .title_value$ab$title,
  note = ("Était jugée aberrante toute valeur numérique différente d'un
    entier compris entre 0 et 4 inclus . {.title_value$ab$note} Aucune valeur
    décimale n'était retrouvée."),
  qmd = TRUE
)

include_graphics("output/fig_value_ab.png")
```

Il y avait un total de **`r nrow(.value_data$na$n)` (`r .value_data$na$p`) valeurs manquantes**, dont voici le détail :
...
#| lightbox:
#|   group: fig-value
.title_value_na <- str_fig(
  title = .title_value$na$title,
  note = .title_value$na$note,
  qmd = TRUE
)

include_graphics("output/fig_value_na.png")
```

::: callout-important
# Gestion des valeurs aberrantes

Les `r nrow(.value_data$ab$n)` valeurs aberrantes présentes dans l'échelle SCL90 ont été converties en valeurs manquantes.
:::

```{r}
.n_obs <- nrow(set_na(scl$base) |> drop_na())
.p_obs <- label_p()(.n_obs / nrow(scl$base))
.sum_ab_na <- nrow(.value_data$ab$n) + nrow(.value_data$na$n)
...
- `r nrow(.value_data$ab$n)` + `r nrow(.value_data$na$n)` = **`r .sum_ab_na` (`r .p_ab_na`) valeurs manquantes**
...
hdrs$locf <- lst(
  data = hdrs$sum$wide |>
    pivot_longer(cols = matches("j"), values_to = "hdrs", names_to = "visit") |>
    group_by(numero) |>
    fill(hdrs) |>
    ungroup() |>
    pivot_wider(names_from = visit, values_from = hdrs) |>
    mutate(delta = j0 - j56) |>
    select(groupe, delta),
  mean = data |>
    tbl_summary(
...
hdrs$lme <- lst(
  ...
  fit = nlme::lme(
    fixed = hdrs ~ groupe * visit,
    random = ~ visit | numero,
    data = data
  ),
  tidy = lst(
    data = tidy(fit, conf.int = TRUE) |> split(~effect),
    fix = data$fixed |>
      merge_estim_ci() |>
      select(term, estimate_ci, p.value),
    ran = data$ran_pars |> select(group, term, estimate)
  )
)
```
...
hdrs$lme$tidy$fix |>
  ...
  gt_qmd() |>
  tab_style(style = cell_text(align = "right"), locations = cells_body(p.value))
```
...
```{r}
.est <- hdrs$lme$tidy$data$fixed$estimate |>
  round(2) |>
  abs()
```
...
.title_surv <- str_fig(
  title = "Comparaison de la probabilité de non-réponse au traitement
...
```{r}
gt_qmd(.hdrs_surv_model_tte$tidy$"0")
```

# [Groupe 1]{.cb1}

```{r}
gt_qmd(.hdrs_surv_model_tte$tidy$"1")
```
...
- Hypothèse de proportionnalité des risques non rejetée au risque $\alpha$ = 5% [(p = `r round(cox.zph(.hdrs_surv_model_cox$obj)$table[2,3], 3)`)]{.sm-em}
