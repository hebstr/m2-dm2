...
lst(data =
      hdrs$sum$long |>
        ...
        group_by(numero) |>
          mutate(bin = if_else(hdrs <= 0.5 * hdrs[visit == 0], 1, 0),
        ...
        distinct(),
    model =
      lst(fit = exprs(Surv(time, event) ~ groupe, data),
          tte =
            ...
            lst(obj = do.call("coxph", fit),
                tidy =
                  tidy(obj,
                       ...
                       conf.int = TRUE) |>
                    merge_estim_ci() |>
                    mutate(str = "Hazard ratio pour l'absence de réponse",
                           p.value =
...
hdrs$surv$model$tte$obj |>
  ...
  geom_text(data = hdrs$surv$model$cox$tidy,
            mapping =
              aes(label =
  ...
  scale_ggsurvfit(x_scales =
                    list(name = "Durée depuis la première visite (jours)",
                         breaks = hdrs$visit$num,
                         limits = c(0, 57)),
                  y_scales =
                    list(name = "Probabilité de non-réponse (%)",
