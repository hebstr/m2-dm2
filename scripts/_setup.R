...
set_hdrs <- \(.visit = NULL) {
  ...
  .hdrs <-
  .base$hdrs |>
    mutate(hamd16 = coalesce(hamd16a, hamd16b),
           .keep = "unused",
           .after = hamd15)
...
hdrs <-
lst(visit =
      lst(num = c(0, 4, 7, 14, 21, 28, 42, 56),
          str = paste0("j", num)),
    ...
    sum =
      lst(long =
            set_hdrs() |>
              rowwise() |>
                mutate(hdrs = sum(across(matches("hamd"))),
                       .after = visit,
                       .keep = "unused") |>
                ungroup() |>
...
lst(base = .base$scl,
    n =
      lst(row = n_distinct(base$numero),
          col = base |> select(matches("q")) |> ncol(),
          rc = nrow(base) * col),
    value =
      lst(data =
            base |>
              filter(!if_all(matches("q"), ~ !!norm(.))) |>
              select(!where(~ unique(!!norm(.)) |> is_true())),
          total =
            base |>
              pivot_longer(cols = matches("q")) |>
              mutate(across(c(visit, value), ~ factor(.))) |>
              filter(!eval(norm(value))),
          ab = drop_na(total),
          na =
            total |>
              filter(is.na(value)) |>
              mutate(visit = str_extract(visit, "(?<=j)\\d+"),
                     value = 1)),
    dim =
      names(.dim) |>
        map(~ base |>
              set_na() |>
              rowwise() |>
                transmute("{.}" := sum(across(.dim[[.]]), na.rm = TRUE))) |>
        list_cbind(),
    join =
      base[c("numero", "visit")] |>
        bind_cols(dim) |>
        left_join(y = hdrs$sum$long,
