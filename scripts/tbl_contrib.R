...
.contrib <- lst(
  pc = list(j0 = c("PC1", "PC2", "PC3"), j56 = "PC1") |>
    imap(
      ~ set_hdrs(.y) |>
        easy_pca() |>
        pca_var_extract() |>
...
easy_sort <- \(x, ..., columns, target_columns = everything()) {
  ...
  x |>
    ...
    tab_style(
      ...
      locations = list(
        cells_column_labels("description"),
...
tbl_contrib_sort <- list(
  j0 = tibble(
    ...
    left_join(.contrib$max, by = join_by(item == column)) |>
    mutate(item = as.character(item)) |>
    relocate(PC, contrib, .before = everything()) |>
    arrange(PC, desc(contrib)) |>
    easy_sort(groupname_col = "PC", columns = contrib),
  j56 = tibble(
    PC1 = round(.contrib$pc$j56$PC1 * 100, 1),
    item = as.character(1:17),
    description = .contrib$labels
  ) |>
    easy_sort(arrange = TRUE, columns = PC1)
)
